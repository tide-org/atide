...
export default class TideRequestHandler {
    ...
    static requestLookup = {
      ...
      'set_config_dictionary_item': (requestValue) => {
        ...
        isBufferCache = ((requestValue.config_dictionary||{}).internal||{}).buffer_caches;
        if (isBufferCache) {
           bufferNames = Object.keys(requestValue.config_dictionary.internal.buffer_caches);
           for (bufferName of bufferNames) {
             WorkspaceHandler.updateEditorByBufferName(bufferName);
           }
        }
      },
